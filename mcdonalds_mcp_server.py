...
mcp = FastMCP(
    "McDonald's China",
    instructions="提供麦当劳中国餐厅查询、菜单浏览、营养计算和促销活动服务"
)
...
@mcp.resource("mcdonalds://menu/{category}")
def get_menu_resource(category: str) -> str:
    """
    以资源形式获取菜单
    URI: mcdonalds://menu/{category}
    """
    return get_menu(category)


@mcp.prompt("order_meal")
def order_meal_prompt(meal_type: str = "汉堡套餐") -> str:
    """
    点餐建议提示词
    
    Args:
        meal_type: 套餐类型，例如：汉堡套餐、早餐、开心乐园餐
    """
    return f"""请帮我推荐一份{meal_type}，需要考虑：
1. 口味偏好（辣/不辣）
2. 预算范围
3. 营养搭配
4. 当前是否有促销活动

请使用可用的麦当劳工具查询相关信息后给出建议。"""
